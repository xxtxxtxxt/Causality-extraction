...
for i in range(len(a)):
    wrongdata=0
    if '原因中的核心名词' in a[i]['key_words'] and a[i]['key_words']['原因中的核心名词']['start']==-1:
        wrongdata+=1
    else:
        x=['O']*len(list(a[i]['text']))
        if '原因中的核心名词' in a[i]['key_words']:
            for j in range(a[i]['key_words']['原因中的核心名词']['start'],a[i]['key_words']['原因中的核心名词']['end']+1):
                if j==a[i]['key_words']['原因中的核心名词']['start']:
                    x[j]='reason_noun_B'
                else:
                    x[j]='reason_noun_I'
        if '原因中的谓语或状态' in a[i]['key_words']:
            for j in range(a[i]['key_words']['原因中的谓语或状态']['start'],a[i]['key_words']['原因中的谓语或状态']['end']+1):
                if j==a[i]['key_words']['原因中的谓语或状态']['start']:
                    x[j]='reason_state_B'
                else:
                    x[j]='reason_state_I'
        if '结果中的核心名词' in a[i]['key_words']:
            for j in range(a[i]['key_words']['结果中的核心名词']['start'],a[i]['key_words']['结果中的核心名词']['end']+1):
                if j==a[i]['key_words']['结果中的核心名词']['start']:
                    x[j]='result_noun_B'
                else:
                    x[j]='result_noun_I'  
        if '结果中的谓语或状态' in a[i]['key_words']:        
            for j in range(a[i]['key_words']['结果中的谓语或状态']['start'],a[i]['key_words']['结果中的谓语或状态']['end']+1):
                if j==a[i]['key_words']['结果中的谓语或状态']['start']:
                    x[j]='result_state_B'
                else:
                    x[j]='result_state_I'   
        temp=(list(a[i]['text']),x)
...
if __name__ == '__main__':
   ...
   #training_data = [("the wall street journal reported today that apple corporation made money".split(),"B I I I O O O B I O O".split()), ("georgia tech is a university in georgia".split(),"B I O O O O B".split())]
    ...
    training_data = training_data1[0:int(len(training_data1)*2/3)]
    word_to_ix = {}
    word_to_ix['<PAD>'] = 0
    ...
    tag_to_ix = {'reason_noun_B': 0, 'reason_noun_I': 1, 'reason_state_B': 2, 'reason_state_I':3,'result_noun_B':4,'result_noun_I':5,'result_state_B':6,'result_state_I':7,'O':8,START_TAG: 9, STOP_TAG: 10, PAD_TAG: 11}
    ...
    with torch.no_grad():
        count=0
        correct=0
        for i in range(int(len(training_data1)*2/3)+1,len(training_data1)):
            precheck_sent = prepare_sequence(training_data1[i][0], word_to_ix)
            tt=[tag_to_ix[i] if i in tag_to_ix else i for i in training_data1[i][1]]
            for j in range(len(training_data1[i][1])):
                count+=1
                if model(precheck_sent)[1][j]==tt[j]:
                    correct+=1
        # We got it!
